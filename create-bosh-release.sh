...
cmd="cd bosh-release && bosh create-release $@"
docker run --mount type=bind,source=${DIR},target=/bosh-release relintdockerhubpushbot/cf-deployment-concourse-tasks /bin/bash -c "${cmd}"
